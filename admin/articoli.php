    ...
    if ($action === 'edit' && $id) {
        if ($db->articleSlugExists($slug, $id)) {
            ...
            if ($db->updateArticle($id, $title, $slug, $content, $excerpt, $category_id, $province_id, $city_id, $status, $featured_image, $gallery_images, $seo_title, $seo_description, $seo_keywords)) {
                ...
                // 🌍 TRADUZIONE AUTOMATICA: Traduci articolo in tutte le lingue supportate
                try {
                    $articleData = [
                        'title' => $title,
                        'content' => $content,
                    ...
                    $translationResult = $translationService->translateArticle($id, $articleData);
                    if ($translationResult) {
                        $message = 'Articolo aggiornato e tradotto automaticamente in tutte le lingue supportate!';
                    } else {
                        $message = 'Articolo aggiornato con successo! Nota: alcune traduzioni potrebbero non essere riuscite.';
                    }
                    $messageType = 'success';
                } catch (Exception $e) {
                    error_log("Errore traduzione articolo $id: " . $e->getMessage());
                    $message = 'Articolo aggiornato con successo! Le traduzioni verranno elaborate in background.';
                    $messageType = 'success';
                }
            } else {
        ...
        if ($db->articleSlugExists($slug)) {
            ...
            if ($articleId) {
                ...
                // 🌍 TRADUZIONE AUTOMATICA: Traduci nuovo articolo in tutte le lingue supportate
                try {
                    $articleData = [
                        'title' => $title,
                        'content' => $content,
                    ...
                    $translationResult = $translationService->translateArticle($articleId, $articleData);
                    if ($translationResult) {
                        $message = 'Articolo creato e tradotto automaticamente in tutte le lingue supportate!';
                    } else {
                        $message = 'Articolo creato con successo! Nota: alcune traduzioni potrebbero non essere riuscite.';
                    }
                    $messageType = 'success';
                } catch (Exception $e) {
                    error_log("Errore traduzione nuovo articolo $articleId: " . $e->getMessage());
                    $message = 'Articolo creato con successo! Le traduzioni verranno elaborate in background.';
                    $messageType = 'success';
                }
            } else {
