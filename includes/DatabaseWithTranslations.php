...
class DatabaseWithTranslations {
    ...
    public function saveArticle($data, $id = null) {
        try {
            // Salva l'articolo utilizzando il metodo parent
            $articleId = parent::saveArticle($data, $id);
    ...
    public function updateArticle($id, $data) {
        try {
            // Aggiorna l'articolo utilizzando il metodo parent
            $success = parent::updateArticle($id, $data);
    ...
    private function triggerArticleTranslation($articleId, $articleData) {
        try {
            // Traduzione sincrona per ora (in produzione si potrebbe usare una coda)
    ...
    private function triggerStaticContentTranslation($staticContentId, $originalText) {
        try {
            ...
            foreach ($supportedLanguages as $langCode => $langInfo) {
                ...
                // Chiama il servizio di traduzione per ogni lingua
    ...
    public function getArticleByLanguage($articleId, $langCode = 'it') {
        return $this->translationService->getArticleTranslation($articleId, $langCode);
    }
    ...
    public function getStaticContent($contentKey, $langCode = 'it') {
        return $this->translationService->getStaticContentTranslation($contentKey, $langCode);
    }
    ...
    public function translateAllExistingArticles() {
        try {
            ...
            foreach ($articles as $article) {
                if ($this->translationService->translateArticle($article['id'], $article)) {
                    $translatedCount++;
                }
    ...
    public function translateAllStaticContent() {
        return $this->translationService->translateStaticContent();
    }
